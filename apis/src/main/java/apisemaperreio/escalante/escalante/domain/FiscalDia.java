...
public class FiscalDia extends ServicoOperacional {
    ...
    public Optional<Militar> verificarCovApto(List<Militar> militares, List<ServicoOperacional> militaresEscalados) {
        var listaVerificacao = militaresEscalados.stream()
                .filter(servico -> servico.getDataServico().equals(this.getDataServico())
                        && (servico.getFuncao().equals(Funcao.COV)
                                || servico.getFuncao().equals(Funcao.OPERADOR_DE_LINHA)))
        ...
        var ehCov = listaVerificacao.stream()
                .anyMatch(servico -> servico.getMatriculaMilitar().equals(maisAntigo.get().getMatricula())
                        && servico.getFuncao().equals(Funcao.COV));
        return ehCov ? maisAntigo : Optional.empty();
    }
